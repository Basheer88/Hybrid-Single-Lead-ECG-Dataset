...
# Data loaders
train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=4, persistent_workers=True)  # Adjust the number of workers
val_loader = DataLoader(val_dataset, batch_size=64, shuffle=False, num_workers=4, persistent_workers=True)    # Adjust the number of workers
...
class CNNClassifier(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        inputs, labels = batch
        outputs = self(inputs)  # Ensure this line is present and correctly assigns the output of the model to 'outputs'
...
def train_and_evaluate(model_class):
    ...
    print(f"{model_class.__name__} Validation Metrics: {trainer.callback_metrics}")
